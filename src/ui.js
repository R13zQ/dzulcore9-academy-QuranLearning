...
class UIManager {
  ...
  initButtons() {
    ...
    if (btnBackArchive) {
      btnBackArchive.addEventListener('click', () => {
        this.showScreen('menu');
      });
    }

    const btnBackCustomizer = document.getElementById('btn-back-customizer');
    if (btnBackCustomizer) {
      btnBackCustomizer.addEventListener('click', () => {
        this.showScreen('menu');
      });
    }

    // Bind HUD buttons
    const hudBtnArchive = document.getElementById('hud-btn-archive');
    if (hudBtnArchive) {
      hudBtnArchive.addEventListener('click', () => {
        this.game.pauseGame();
        this.showScreen('archive');
        this.renderArchive();
      });
    ...
    const hudBtnCustomizer = document.getElementById('hud-btn-customizer');
    if (hudBtnCustomizer) {
      hudBtnCustomizer.addEventListener('click', () => {
        this.game.pauseGame();
        this.showScreen('customizer');
        this.renderCustomizer();
      });
    ...
    const hudBtnHome = document.getElementById('hud-btn-home');
    if (hudBtnHome) {
      hudBtnHome.addEventListener('click', () => {
        this.game.exitToMenu();
      });
    }

    // Mobile interact prompt overlay click helper
    const interactPrompt = document.getElementById('interact-prompt');
    if (interactPrompt) {
      interactPrompt.addEventListener('click', () => {
        this.game.controls.triggerInteract();
      });
      // Enable mouse click on prompt
  ...
  showMushafModal(stage) {
    ...
    document.getElementById('btn-close-mushaf').addEventListener('click', () => {
      modal.remove();
    });
  }
