...
class UIManager {
  ...
  bindElement(el, callback) {
    ...
    const trigger = (e) => {
      const now = Date.now();
      if (now - lastTriggerTime < 300) return; // Debounce double triggers
      lastTriggerTime = now;
    ...
    // Bind standard click (covers mouse click and emulated taps)
    el.addEventListener('click', trigger);
    
    // Bind touch events directly to be extremely responsive on touch devices
    el.addEventListener('touchend', (e) => {
      e.preventDefault(); // Prevent double click event emulation
      trigger(e);
    }, { passive: false });
  }
  ...
  initButtons() {
    ...
    this.bindButton('hud-btn-toggle-touch', () => {
    ...
    if (interactPrompt) {
      this.bindElement(interactPrompt, () => {
  ...
  toggleTouchControls() {
    this.game.controls.isTouchDevice = !this.game.controls.isTouchDevice;
    const mobileControls = document.querySelector('.mobile-controls');
    if (mobileControls) {
      if (this.game.controls.isTouchDevice) {
        mobileControls.classList.remove('hidden');
        this.showNotification("Kontrol layar sentuh (joystick) diaktifkan!", "success");
      } else {
        mobileControls.classList.add('hidden');
        this.showNotification("Kontrol layar sentuh dinonaktifkan.", "incorrect");
      }
    }
  }
  ...
  showScreen(screenId) {
    ...
    // Dynamic pointer-events toggling on ui-container is now handled purely via CSS in style.css to avoid browser hit-test bugs.
  }
  ...
  openPuzzle(puzzleData, questionIndex, totalQuestions, onCorrect, onIncorrect) {
    ...
    this.timerInterval = setInterval(() => {
      ...
      if (timeLeft <= 0) {
        ...
        if (this.game && this.game.audio) this.game.audio.playIncorrect();
    ...
    puzzleData.options.forEach((optionText, index) => {
      ...
      this.bindElement(btn, () => {
        ...
        if (isCorrect) {
          ...
          if (this.game && this.game.audio) this.game.audio.playCorrect();
