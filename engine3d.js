...
class Engine3D {
  ...
  init() {
    ...
    // Disabled antialiasing and added high-performance power preference for faster mobile rendering
  ...
  initMouseOrbit() {
    ...
    // Handle mouse drag to rotate camera yaw/pitch
    ...
    this.container.addEventListener('touchstart', (e) => {
      // Ignore if clicking virtual control regions
      const x = e.touches[0].clientX;
      const y = e.touches[0].clientY;
      const joystickZone = document.getElementById('joystick-zone');
      const jumpBtn = document.getElementById('jump-btn');
      
      if (joystickZone && joystickZone.getBoundingClientRect().left < x && x < joystickZone.getBoundingClientRect().right &&
          joystickZone.getBoundingClientRect().top < y && y < joystickZone.getBoundingClientRect().bottom) {
        return;
      }
      if (jumpBtn && jumpBtn.getBoundingClientRect().left < x && x < jumpBtn.getBoundingClientRect().right &&
          jumpBtn.getBoundingClientRect().top < y && y < jumpBtn.getBoundingClientRect().bottom) {
        return;
      }
